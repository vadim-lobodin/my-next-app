...
const ChatIsland = React.forwardRef<HTMLDivElement, ChatIslandProps>(
  ({ className, messages: externalMessages, taskLabel, taskProgress, contextPreview, chatInputProps, children, ...props }, ref) => {
    ...
    }, [messages.length, contextPreview])
    ...
    return (
      <div
        className={cn(
          "bg-[var(--fleet-island-background)] text-[var(--fleet-text-primary)] rounded-[var(--fleet-radius-lg)] overflow-hidden flex flex-col h-full @container",
          className
        )}
        ref={ref}
        {...props}
      >
        ...
        <div className="flex-shrink-0 flex flex-col max-w-[600px] mx-auto w-full pb-0 @[600px]:pb-3">
          {/* Task progress bar */}
          {taskLabel && (
            <div
