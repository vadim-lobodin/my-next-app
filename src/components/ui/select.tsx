...
function SelectTrigger({
  ...
  error,
  ...props
}: React.ComponentProps<typeof SelectPrimitive.Trigger> & { error?: boolean }) {
  return (
    <SelectPrimitive.Trigger
      className={cn(
        // Base — matches Air inputField styling
        "flex w-full items-center justify-between gap-1",
        "h-7 rounded-[var(--fleet-radius-sm)] border px-2 py-1",
  ...
  ({
    ...
    return (
      <Select value={value} defaultValue={defaultValue} onValueChange={onValueChange} disabled={disabled}>
        <SelectTrigger ref={ref} className={className} error={error}>
